...
export default function ReportDetailPage() {
  ...
  const canEdit =
    user && report && report.createdBy && user._id === report.createdBy._id;
  const canResolve = user && user.isAdmin && report && !report.resolved;
  const canDelete =
    user &&
    report &&
    (user.isAdmin || (report.createdBy && user._id === report.createdBy._id));
  ...
  const handleAdminToggleResolved = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok)
        throw new Error(data.error || "Failed to update resolved status");
      await refetchReport();
    } catch (err) {
  ...
  const userReview =
    user && reviews.find((r) => r.author && r.author._id === user._id);
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <div className="mb-2 text-sm text-gray-500">
            ...
            <span className="font-medium text-gray-700">
              {report.rawLocation || "Unknown location"}
            </span>
          </div>
                    ...
                    {images.map((_, i) => (
                      <span
                        ...
                        className={`inline-block w-2 h-2 rounded-full ${
                          i === imgIdx ? "bg-green-600" : "bg-gray-300"
                        }`}
                      ></span>
                    ))}
                  </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 h-full flex flex-col">
            ...
            <form onSubmit={handleAddReview} className="mb-6">
              ...
              <div className="flex items-center gap-2 mb-2">
                <label className="flex items-center gap-1 text-black">
                  <input
                    type="checkbox"
                    checked={reviewUpvote}
                    onChange={(e) => setReviewUpvote(e.target.checked)}
                    disabled={reviewSubmitting}
                  />
                  <span>Upvote</span>
                </label>
              ...
              {reviewError && (
                <div className="text-red-600 text-sm mb-2">{reviewError}</div>
              )}
              <button
                type="submit"
          ...
          {user && userReview && editingReviewId !== userReview._id && (
            <div className="mb-4 text-sm text-green-700">
              You have already reviewed this report.
            </div>
          )}
          {/* Edit review form */}
          {editingReviewId && (
            <form
              onSubmit={(e) => {
                e.preventDefault();
                handleSaveEditReview(editingReviewId);
              }}
              className="mb-6"
            >
              <textarea
              ...
              <div className="flex items-center gap-2 mb-2">
                <label className="flex items-center gap-1 text-black">
                  <input
                    type="checkbox"
                    checked={editingReviewUpvote}
                    onChange={(e) => setEditingReviewUpvote(e.target.checked)}
                    disabled={reviewSubmitting}
                  />
                  <span>Upvote</span>
                </label>
              ...
              <button
                type="submit"
                className="px-4 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 mr-2"
                disabled={reviewSubmitting}
              >
                Save
              </button>
          ...
          <div className="flex-1 overflow-y-auto space-y-4">
            ...
            {reviews.map((review) => {
              const isReviewAuthor =
                user && review.author && review.author._id === user._id;
              const isReportOwner =
                user && report.createdBy && user._id === report.createdBy._id;
              return (
                <div
                  key={review._id}
                  className="bg-gray-50 rounded p-3 border flex flex-col gap-1"
                >
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    >
                      {review.upvote ? "Upvoted" : "No Upvote"}
                    </span>
                    <span className="text-xs text-gray-400 ml-auto">
                      {new Date(review.createdAt).toLocaleString()}
                    </span>
                  </div>
                  <div className="text-black text-sm mb-1 whitespace-pre-line">
                    {review.comment}
                  </div>
